...
static inline int
bpf_object__create_skeleton(struct fsslower_bpf *obj, char * obj_buf, size_t obj_buf_sz)
{       
...
int main(int argc, char **argv)
{
	...
	if (err) {
        ...
        skel = (struct fsslower_bpf *)calloc(1, sizeof(*skel));
        if (!skel) { 
                errno = ENOMEM; 
                goto cleanup;
        }
        ...
        err = bpf_object__open_skeleton(skel->skeleton, &open_opts);
	if (!skel) {
	 ...
	 */
	err = support_fentry ? bpf_object__attach_skeleton(skel->skeleton) : attach_kprobes(skel);
	if (err) {
