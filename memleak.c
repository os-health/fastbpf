...
static void disable_kernel_node_tracepoints(struct object_bpf *skel);
static void disable_kernel_percpu_tracepoints(struct object_bpf *skel);
static void disable_kernel_tracepoints(struct object_bpf *skel);

static int attach_uprobes(struct object_bpf *skel);
...
int main(int argc, char *argv[])
{
	...
	libbpf_set_print(libbpf_print_fn);
        ...
        ret = bpf_object__create_skeleton(skel, obj_buf, obj_buf_sz);
        if (ret)
                goto cleanup;

        ret = bpf_object__open_skeleton(skel->skeleton, NULL);
	if (ret) {
	...
	ret = bpf_object__load_skeleton(skel->skeleton);
	if (ret) {
	...
	ret = bpf_object__attach_skeleton(skel->skeleton);
	if (ret) {
...
void disable_kernel_node_tracepoints(struct object_bpf *skel)
{
...
void disable_kernel_percpu_tracepoints(struct object_bpf *skel)
{
...
void disable_kernel_tracepoints(struct object_bpf *skel)
{
...
int attach_uprobes(struct object_bpf *skel)
{
